...
let filt = document.getElementById('search');
let filter = document.getElementById('search').value.toLowerCase();
filt.addEventListener("keyup", searchfun);
pageUl.onclick = function () {
	if (filter != "") {
		searchfun();
	}
}

function deleteRow(mobileNumber) {
	fetch('/deleteRow', {
	  method: 'POST',
	  headers: {
	...
	.then(data => {
	  ...
	  if (data.success) {
		window.location.href = '/donor_list_index';
	  } else {
